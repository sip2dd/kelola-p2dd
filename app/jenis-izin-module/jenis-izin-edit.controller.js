(function () {
    ...
    angular
        ...
        .controller('JenisIzinEditController', JenisIzinEditController);

    /* @ngInject */
    function JenisIzinEditController(
        $http, $state, EnvironmentConfig, $stateParams, $mdDialog, $mdMedia, $scope, $log,
        $document, AppFactory, JenisIzinFactory, JenisDokumenFactory, PenomoranFactory, _
    ) {
        var vm = this;
        ...
        JenisIzinFactory.getJenisIzin(id).then(function (res) {
            ...
            if (!_.isNull(vm.jenis_izin.jenis_dokuman)) {
                vm.searchTextJenisDokumenIzin = vm.jenis_izin.jenis_dokuman.deskripsi;
            }
        ...
        // Dialog for Overriding
        vm.showOverrideAlurWindow = function (index) {
            var useFullScreen = ($mdMedia('sm') || $mdMedia('xs')) && $scope.customFullscreen;
            $mdDialog.show({
                    controller: OverrideAlurDialogController,
                    templateUrl: 'app/jenis-izin-module/override-alur-dialog.tmpl.html',
                parent: angular.element($document.body),
                    // targetEvent: ev,
                    clickOutsideToClose: true,
                    fullscreen: useFullScreen,
                    locals: {
                        alurProsesData: vm.jenis_pengajuan[index].alur_prose,
                        jenisPengajuanId: vm.jenis_pengajuan[index].id
                    }
                })
                .then(function (dialogData) {
                    vm.jenis_pengajuan[index].alur_prose = dialogData;
                });
